        ...
        viewport:
            ...
            frame:
                ...
                vbox:
                    ...
                    for item in store_items:
                        if item.isVisible():
                            text item.display_name + ("    [[owned:{!s}]".format(eval(item.variable_name)) if item.variable_name != None else "") font "Font/NotoSansCJKsc-Regular.otf" size 20 first_indent 50 line_spacing 38 color "#0a0a0a"
                vbox:
                    ypos 12
                    for item in store_items:
                        if item.isVisible():
                            text str(item.cost) font "Font/NotoSansCJKsc-Regular.otf" size 20 first_indent 685 line_spacing 38 color "#0a0a0a"
    ...
    frame:
        ...
        for item in store_items:
            if BM.hovered == item.id:
                text item.display_name xpos 50 ypos 50 size 35 font "Font/NotoSansCJKsc-Regular.otf" color '000'
                text item.tooltip xpos 50 ypos 150 size 20 font "Font/NotoSansCJKsc-Regular.otf" color '000'
